...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'model_data/ep100-loss0.078-val_loss0.062.pth',
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            restored_image = outputs[-1].cpu()
            restored_image = restored_image[0]
            restored_image = np.array(restored_image, dtype='float32')
            restored_image = np.transpose(restored_image, (1, 2, 0))
            restored_image = list(restored_image * 255)
            restored_image = np.array(restored_image, dtype='uint8')
