...
if __name__ == "__main__":
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        tqdm.write("Load model done.")

        tqdm.write("Get predict result.")
        for corruption in corruption_tuple[9:10]:
            for severity in severity_tuple:
                for image_id in tqdm(image_ids, desc=f'{corruption} {severity}'):
                    image_path = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages-corrupted/", corruption, severity,
                                              image_id + ".jpg")
                    image = Image.open(image_path)
                    if map_vis:
                        image.save(os.path.join(map_out_path,
                                                "images-optional/" + corruption + "_" + severity + "_" + image_id + ".jpg"))
                    yolo.get_map_txt(corruption + "_" + severity + "_" + image_id, image, class_names, map_out_path)
        tqdm.write("Get predict result done.")

    if map_mode == 0 or map_mode == 2:
        tqdm.write("Get ground truth result.")
        for corruption in corruption_tuple[9:10]:
            for severity in severity_tuple:
                for image_id in tqdm(image_ids, desc=f'{corruption} {severity}'):
                    with open(os.path.join(map_out_path, "ground-truth/" + corruption + "_" + severity + "_" + image_id + ".txt"), "w") as new_f:
        ...
        tqdm.write("Get ground truth result done.")
